...
def build_video_metadata_kvrocks_client(settings: Settings):
    hosts = [
        host.strip()
        for host in settings.video_metadata_kvrocks_hosts
        if str(host).strip()
    ]
    if not hosts:
        return None
    ...
    common_kwargs = {
        "host": hosts[0],
        "port": settings.video_metadata_kvrocks_port,
        "password": settings.video_metadata_kvrocks_password or None,
        "decode_responses": True,
    }
    tls_kwargs = _build_external_tls_kwargs(
        tls_enabled=settings.video_metadata_kvrocks_tls_enabled,
        ca_cert=settings.video_metadata_kvrocks_ssl_ca_cert,
        client_cert=settings.video_metadata_kvrocks_ssl_client_cert,
        client_key=settings.video_metadata_kvrocks_ssl_client_key,
        prefix="video-metadata",
    )

    if settings.video_metadata_kvrocks_cluster_enabled:
        client = RedisCluster(
            **common_kwargs,
            **tls_kwargs,
        )
    else:
        client = redis_async.Redis(**common_kwargs, **tls_kwargs)

    return client


def _build_external_tls_kwargs(
    *,
    tls_enabled: bool,
    ca_cert: str,
    client_cert: str,
    ...
    cert_paths = _materialize_tls_files(
        prefix=prefix,
        ca_cert=ca_cert,
...
def _materialize_tls_files(
    *,
    ...
    def _write_if_present(filename: str, content: str) -> str | None:
        ...
        if not normalized:
            return None
        path = TLS_CERT_DIR / f"{prefix}-{filename}"
        path.write_text(normalized + "\n")
        os.chmod(path, 0o600)
        return str(path)

    return {
        "ca": _write_if_present("ca.crt", ca_cert),
        "cert": _write_if_present("client.crt", client_cert),
        "key": _write_if_present("client.key", client_key),
    }
